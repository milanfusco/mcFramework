...
def heterogeneous_1d_slab(u235_material, water_material):
    ...
    return Geometry1DSlab(0.0, 10.0, [0.0, 2.0, 8.0, 10.0], [water_material, u235_material, water_material])


@pytest.fixture
def simple_2d_plane(u235_material, water_material):
    """Fixture providing 2D plane with fuel center and water reflector."""
    return Geometry2DPlane(0.0, 10.0, 0.0, 10.0, [(2.0, 2.0, 8.0, 8.0, u235_material)], water_material)
...
class TestGeometry2DPlane:
    ...
    def test_initialization_valid(self, u235_material, water_material):
        ...
        geom = Geometry2DPlane(0.0, 10.0, 0.0, 10.0, [(2.0, 2.0, 8.0, 8.0, u235_material)], water_material)
        assert geom.x_min == 0.0
        assert geom.x_max == 10.0
...
class TestNeutronState:
    ...
    def test_custom_initialization(self):
        ...
        neutron = NeutronState(x=3.0, y=4.0, mu=0.5, energy_group=0, weight=0.8, alive=False)
...
class TestNeutronTransportSimulation:
    ...
    def test_single_simulation_with_overrides(self, simple_1d_slab, u235_material):
        ...
        result = sim.single_simulation(
            geometry=new_geom, source_position=(2.5,), source_energy_group=1, flux_bins=25
        )
        assert isinstance(result, float)
...
class TestKEFFSimulation:
    ...
    def test_reflective_boundaries(self, u235_material):
        ...
        k_eff_vacuum = sim_vacuum.single_simulation() # noqa: F841 # used for debugging

        # Reflective boundaries
        geom_reflect = Geometry1DSlab(
            0.0, 10.0, [0.0, 10.0], [u235_material], boundary_condition="reflective"
        )
    ...
    def test_keff_with_parameter_overrides(self, simple_1d_slab):
        ...
        sim = KEFFSimulation(geometry=simple_1d_slab, n_generations=15, n_neutrons_per_generation=50)
        sim.set_seed(42)
        k_eff = sim.single_simulation(n_generations=10, n_neutrons_per_generation=30, skip_generations=3)
        assert isinstance(k_eff, float)
        assert k_eff >= 0
    ...
    def test_keff_population_exactly_matches_target(self, u235_material):
        ...
        sim = KEFFSimulation(geometry=geom, n_generations=3, n_neutrons_per_generation=50)
        sim.set_seed(999)
        k_eff = sim.single_simulation()
        assert isinstance(k_eff, float)
...
class TestNeutronTransportIntegration:
    ...
    def test_probabilities_sum(self, simple_1d_slab):
        ...
        result_leak = sim.run(
            n_simulations=100, parallel=False, return_type="leakage_prob", compute_stats=False
        )
        sim.set_seed(42)
        result_abs = sim.run(
            n_simulations=100, parallel=False, return_type="absorption_prob", compute_stats=False
        )
    ...
    def test_framework_integration(self, simple_1d_slab):
        ...
        result = framework.run_simulation(
            "NeutronTest", n_simulations=10, parallel=False, compute_stats=False
        )
        assert result.n_simulations == 10
    ...
    def test_keff_multiple_generations_convergence(self, u235_material):
        ...
        geom = Geometry1DSlab(0.0, 10.0, [0.0, 10.0], [u235_material], boundary_condition="reflective")
        sim = KEFFSimulation(geometry=geom, n_generations=50, n_neutrons_per_generation=100)
        sim.set_seed(42)
    ...
    def test_keff_many_runs_to_hit_edge_cases(self, u235_material):
        ...
        geom = Geometry1DSlab(0.0, 10.0, [0.0, 10.0], [u235_material], boundary_condition="reflective")
        sim = KEFFSimulation(geometry=geom, n_generations=10, n_neutrons_per_generation=30)
