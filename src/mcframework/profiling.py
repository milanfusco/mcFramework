    ...
    **Chunking behavior**: With chunking enabled (default), a simulation of
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "PyTorch profiler requires PyTorch. "
                "Install with: pip install mcframework[gpu]"
            )
    ...
    def _trace_handler(self, prof: Any) -> None:
        ...
        sort_key = (
            "cuda_time_total"
            if "cuda" in self._config.activities
            else "cpu_time_total"
        )
    ...
    def _calculate_chunk_size(self, n_simulations: int) -> int:
        ...
        total_steps = (
            self._config.schedule_wait +
            self._config.schedule_warmup +
            self._config.schedule_active
        )
    ...
    def run(
        ...
        **simulation_kwargs: Any,
        ...
        mps_ctx = (
            self._torch.mps.profiler.profile()
            if use_mps_profiler
            else nullcontext()
        )
        ...
        logger.info(
            "Running %d simulations in %d chunks of ~%d for profiling",
            n_simulations, n_chunks, chunk_size
        )
        ...
        with mps_ctx:
            with self._create_profiler() as prof:
                for i, chunk_seed in enumerate(chunk_seeds):
                    ...
                    logger.debug(
                        "Completed chunk %d/%d (%d/%d simulations)",
                        i + 1, n_chunks, completed, n_simulations
                    )
